...
dados = {'nome': '', 'renach': '', 'cpf': '', 'dataNascimento': '', 'local': '', 'cfc': '', 'resultado': '', 'data': '',
         'hora': ''}
...
def get_dados(contents):  # Regex tags XML
    ...
    dataNascimento = re.search('<dataNascimento>(.*)</dataNascimento>', contents).group(0).replace('<dataNascimento>',
                                                                                                   '').replace(
        '</dataNascimento>', '')
    ...
    resultado = re.search('<resultado>(.*)</resultado>', contents).group(0).replace('<resultado>', '').replace(
        '</resultado>', '')
    ...
    dados['nome'], dados['renach'], dados['cpf'], dados['dataNascimento'], dados['local'], dados['cfc'], dados[
        'resultado'], dados['data'], dados[
        'hora'] = nome, renach, cpf, dataNascimento, local, cfc, resultado, data, hora


if __name__ == "__main__":
    if len(sys.argv) > 1:
        configs = {'api_getran': 'http://getran.detran.df.gov.br/getranServicos/ConsultaHabilitacaoWS?wsdl',
                   'cpf': sys.argv[1], 'token': 'TKN832JHGSI2937MNGHSJ9987', 'user_agent': 'ksoap2-android/2.6.0+'}
        ...
        headers = {'User-Agent': configs['user_agent'], 'SOAPAction': 'http://getran.detran.df.gov.br/getranServicos/',
        ...
        print
        "[ ! ] Executar o script junto com o cpf sem caracter: ./getran.py <cpf> | ./getran.py 1234567891011"
